...
class TechFlexWebsite(http.Controller):
    """Public website routes for the premium TechFlex TT brand site."""
    ...
    def techflex_services(self, **kw):
        return request.render('techflex_tt_brand_site.page_services')
    ...
    def submit_contact_request(self, **post):
        ...
        request.env['crm.lead'].sudo().create({
            ...
            'phone': post.get('phone') or False,
            'description': "\n".join([
                f"Service Interest: {post.get('service_interest', '')}",
                '',
                post.get('message') or '',
            ]),
            'type': 'lead',
